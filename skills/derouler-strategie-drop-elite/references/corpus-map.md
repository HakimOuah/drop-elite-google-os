...
- `raw/vimeo/` : VTT Vimeo et métadonnées minimisées du manifest courant.
- `raw/youtube/` : VTT YouTube et métadonnées minimisées du manifest courant.
...
- `derived/coach-source-index.md` : index sémantique des 229 contenus parlés, source par source.
...
Les 66 contenus du premier lot sont `ASSIMILE_TEXTE` ou `ASSIMILE_TEXTE_QUALITE_LIMITEE`; les 163 contenus du versement complet restent `INGERE_TEXTE_BRUT` jusqu'à relecture fine. Aucun ne devient automatiquement `RELUE_AUDIO` : une lecture sémantique ne corrige pas les erreurs de sous-titrage et ne valide pas les affirmations enseignées.
