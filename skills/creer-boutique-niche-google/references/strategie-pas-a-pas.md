...
- Compte Ads en mode expert (devise/fuseau irréversibles, code promo de lancement, 2FA). MC via **CSS tiers** — voir `docs/css-shopping-france.md` (vérifié le 08/08/2026, sources officielles). **Trois corrections au corpus** : ① le « −20 % » n'est **pas une remise** — Google déduit un pourcentage fixe de chaque enchère quand on passe par son propre CSS ; via un tiers, toute l'enchère entre, soit ≈ **+25 % de puissance d'enchère**, gain réel variable selon la concurrence ; ② **un CSS ne change RIEN aux politiques Merchant Center** et n'accorde **aucune immunité** — Google n'interdit pas le dropshipping, il exige la transparence sur l'exécution (`EXCLU_SYSTEME` : présenter le CSS comme un contournement) ; ③ le partenaire cité par la formation (« Deshops », 39,90 à 69 €/mois) est **introuvable dans l'annuaire officiel** et son domaine est parqué : l'annuaire liste **90 CSS partenaires pour la France**, dont des offres **0-30 €/mois** en self-service. S'auto-CSS est hors de portée d'un marchand (Google exige un comparateur public couvrant ≥ 50 domaines marchands distincts par pays). Reco : démarrer sur Google Shopping le temps de sécuriser la conformité, puis basculer sur un CSS tiers **self-service ≤ 30 €/mois** (jamais « managé » : risque de perdre la main sur ses comptes) — le transfert se révoque en un clic depuis Merchant Center — n'ouvrir le compte que **site 100 % propre** : l'examen commence dès la création. Puis **gel du site ~8 jours** (robot à J+2, humain vers J+7/8). 2e refus = blocage : procéder par élimination, jamais griller un examen.
